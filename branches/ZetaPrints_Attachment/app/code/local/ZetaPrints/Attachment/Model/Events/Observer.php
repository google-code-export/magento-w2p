...
class ZetaPrints_Attachment_Model_Events_Observer
{
  ...
  public function addAttachemntsToOrder($observer)
  {
    ...
    foreach (Mage::getSingleton('core/session')->getData() as $key => $value) {
      if (strpos($key, ZetaPrints_Attachment_Model_Attachment::ATT_SESS) !== false) { // this is attachment data
        if ($value[ZetaPrints_Attachment_Model_Attachment::PR_ID] == $product_id) { // and is for current product
          $attachments[] = $value;
    ...
    foreach ($attachments as $data) {
      $atCollection = Mage::getModel('attachment/attachment')->getAttachmentCollection($data); // get all uploads for give option
      foreach ($atCollection as $att) { // collect all uploads in common array
        $orderAtt[$att->getProductId()][$att->getOptionId()][] = unserialize($att->getAttachmentValue());
      }
    }
  ...
  public function storeAttachments($observer)
  {
    ...
    if (isset($prid, $hash)) {
      ...
      foreach ($hash as $opt_id => $hash_value) {
        $key = ZetaPrints_Attachment_Model_Attachment::ATT_SESS . '_' . $hash_value; // make unique option key
        $sess->setData($key, array ( // save it in session
            ZetaPrints_Attachment_Model_Attachment::PR_ID => $prid,
            ZetaPrints_Attachment_Model_Attachment::OPT_ID => $opt_id,
            ZetaPrints_Attachment_Model_Attachment::ATT_HASH => $hash_value
        ));
      }
    }
  }
