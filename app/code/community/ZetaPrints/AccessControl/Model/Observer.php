...
class ZetaPrints_AccessControl_Model_Observer extends Mage_Core_Model_Abstract
{
  ...
  //  if (! Mage::helper('accesscontrol')->is_extension_enabled || $this->_isApiRequest()) return;
  //      $collection = $observer->getCollection();
  //  Mage::helper('accesscontrol')->filter_out_categories($collection);
  //}
  ...
  public function catalogCategoryCollectionLoadAfter ($observer) {
    if (!Mage::helper('accesscontrol')->is_extension_enabled || $this->_isApiRequest())
      return;

    Mage::helper('groupscatalog')->filter_out_categories($observer->getCategoryCollection());
  }
  ...
  /*public function adminhtmlCustomerSaveAfter($observer)
  {
    ...
    if ($visibleProducts && $customer && $customer->getId())
    {
      ...
      foreach ($visibleProducts as $productState)
      {
        ...
        if ($product->getId())
        {
          Mage::helper('accesscontrol')->setProductAccessibilityForGroup($product, $customer->getGroupId(), (bool) $isAccessible);
        }
      }
    }
  }*/
