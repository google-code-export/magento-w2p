...
class ZetaPrints_Fixedprices_Helper_Data extends Mage_Core_Helper_Abstract
{
  ...
  public function getFixedPrice(Mage_Catalog_Model_Product $product, $qty)
  {
    ...
    $prices = $this->_getFixedPrices($product);

    if(is_array($prices)){
        $id = $this->_getUsedPriceId($product);
        if($id) {
            foreach($prices as $price) {
                if($price['price_id'] == $id) {
                    return $price['price'] / $price['price_qty'];
                }
            }
        }
    ...
    protected function _getUsedPriceId(Mage_Catalog_Model_Product $product)
    {
        $buyRequest = unserialize($product->getCustomOption('info_buyRequest')->getValue());
        $id = false;
        if($buyRequest && isset($buyRequest['fixed-price-id'])) {
            $id = $buyRequest['fixed-price-id'];
        }
        return $id;
    }

    protected function _getFixedPrices(Mage_Catalog_Model_Product $product)
    {
        $prices = $product->getData(self::FIXED_PRICE);
        if ($prices && !is_array($prices)) { // if no complete load is done, FQ will be decimal and not an array. So we have to load it.
  ...
  public function getFixedUnits(Mage_Catalog_Model_Product $product, $qty)
  {
    ...
    if($id = $this->_getUsedPriceId($product)) {
        foreach($prices as $price) {
            if($price['price_id'] === $id) {
                return $price['units'];
            }
